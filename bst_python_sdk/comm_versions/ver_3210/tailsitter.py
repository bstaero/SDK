...
class FlightControlParameters:
	...
	def parse(self,buf):
		...
		self.transition_fwd_rate = struct.unpack_from('<f',buf,offset)[0] / 1
		offset = offset + struct.calcsize('<f')

		self.transition_fwd_angle = struct.unpack_from('<f',buf,offset)[0] / 1
		offset = offset + struct.calcsize('<f')

		self.transition_hover_rate = struct.unpack_from('<f',buf,offset)[0] / 1
		offset = offset + struct.calcsize('<f')
