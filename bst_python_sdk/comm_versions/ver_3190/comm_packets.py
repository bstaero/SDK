...
class Timeout:
	...
	def parse(self,buf):
		...
		self.seconds = struct.unpack_from('<B',buf,offset)[0] / 1
		offset = offset + struct.calcsize('<B')
...
class TelemetryControl:
	...
	def parse(self,buf):
		...
		self.yaw = struct.unpack_from('<f',buf,offset)[0] / 1
		offset = offset + struct.calcsize('<f')
		...
		self.vrate = struct.unpack_from('<f',buf,offset)[0] / 1
		offset = offset + struct.calcsize('<f')
		...
		for i in range(0,16):
			self.actuators.append(struct.unpack_from('<f',buf,offset)[0] / 1)
			offset = offset+struct.calcsize('<f')
...
class TelemetryPosition:
	...
	def parse(self,buf):
		...
		self.height = struct.unpack_from('<f',buf,offset)[0] / 1
		offset = offset + struct.calcsize('<f')
