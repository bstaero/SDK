...
class S0Sensors:
	PACKET_TYPES = ['PAYLOAD_S0_SENSORS']
	SIZE = 36
	...
	def parse(self,buf):
		...
		for i in range(0,2):
			self.static_pressure.append(struct.unpack_from('<I',buf,offset)[0] / 10)
			offset = offset+struct.calcsize('<I')
		...
		self.ground_temperature = struct.unpack_from('<h',buf,offset)[0] / 100
		offset = offset + struct.calcsize('<h')

		self.u = struct.unpack_from('<h',buf,offset)[0] / 100
		offset = offset + struct.calcsize('<h')

		self.v = struct.unpack_from('<h',buf,offset)[0] / 100
		offset = offset + struct.calcsize('<h')

		self.w = struct.unpack_from('<h',buf,offset)[0] / 100
		offset = offset + struct.calcsize('<h')
...
class TelemetryPayload:
	...
	def parse(self,buf):
		...
		self.percent_complete = struct.unpack_from('<B',buf,offset)[0] / 1
		offset = offset + struct.calcsize('<B')
