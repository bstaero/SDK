...
class MHP:
	...
	def __init__ (self, error_code = 0, system_time = 0.0, static_pressure = 0.0,
	dynamic_pressure = [None] * 5, air_temperature = 0.0, humidity = 0.0,
	gyroscope = [None] * 3, accelerometer = [None] * 3,
	magnetometer = [None] * 3, alpha = 0.0, beta = 0.0, q = 0.0, ias = 0.0,
	tas = 0.0):
...
class GPS:
	...
	course = 0.0, satellites = 0, hdop = 0.0, last_fix = -1.0, velocity = 0):
		self.system_time = system_time
		self.week = week
		self.hour = hour
		self.minute = minute
	...
	def parse(self,buf):
		...
		self.hdop = struct.unpack_from('<f',buf,offset)[0]
		offset = offset + struct.calcsize('<f')
	...
	def serialize(self):
		...
		buf.extend(struct.pack('<f', self.hdop))
...
class BatteryState:
	PACKET_TYPES = ['SYSTEM_BATTERY_STATE']
	SIZE = 40

	def __init__ (self, system_time = 0.0, voltage = [None] * 6,
	temperature = [None] * 3):
		self.system_time = system_time

		if (len(voltage) != 6):
			raise ValueError('array voltage expecting length '+str(6)+' got '+str(len(voltage)))

		self.voltage = list(voltage)

		if (len(temperature) != 3):
			raise ValueError('array temperature expecting length '+str(3)+' got '+str(len(temperature)))

		self.temperature = list(temperature)

	def parse(self,buf):
		if (len(buf) != self.SIZE):
			raise BufferError('INVALID PACKET SIZE [BatteryState]: Expected=' + str(self.SIZE) + ' Received='+ str(len(buf)))
		...
		for i in range(0,6):
			self.voltage.append(struct.unpack_from('<f',buf,offset)[0])
			offset = offset+struct.calcsize('<f')

		self.temperature = [];

		for i in range(0,3):
			self.temperature.append(struct.unpack_from('<f',buf,offset)[0])
			offset = offset+struct.calcsize('<f')
...
class TelemetrySystem:
	...
	hour = 0, minute = 0, seconds = 0.0, satellites = 0, pdop = 0.0, rssi = 0,
	lost_comm = 0, lost_gps = 0, engine_on = 0, error_code = 0,
	autopilot_mode = AutopilotMode(0), flight_mode = FlightMode(0)):
		self.batt_voltage = batt_voltage
		self.batt_current = batt_current
		self.batt_coulomb_count = batt_coulomb_count
		self.batt_percent = batt_percent
