...
class FlightControlParameters:
	...
	tecs_max_vx_dot = 0.0, nav_lookahead = 0.0, tuning_ias = 0.0,
	max_height_error_mode = 0.0, max_v_error_mode = 0.0,
	k_height_tracking = 0.0, k_flare = 0.0, k_land = 0.0, k_cruise = 0.0,
	k_climb = 0.0, k_speed_hold = 0.0):
...
class EstimatorParameters:
	...
	def __init__ (self, cf_filter_gain = 0.0, pressure_drift_gain = 0.0,
	ax_filter_alpha = 0.0, ax_filter_beta = 0.0, ay_filter_alpha = 0.0,
	ay_filter_beta = 0.0, turnrate_filter_alpha = 0.0,
	turnrate_filter_beta = 0.0, altitude_filter_Ph = 0.0,
	altitude_filter_R = 0.0, altitude_filter_Q1 = 0.0, altitude_filter_Q2 = 0.0,
	altitude_filter_Q3 = 0.0):
		self.cf_filter_gain = cf_filter_gain
		self.pressure_drift_gain = pressure_drift_gain
		self.ax_filter_alpha = ax_filter_alpha
		self.ax_filter_beta = ax_filter_beta
		self.ay_filter_alpha = ay_filter_alpha
		self.ay_filter_beta = ay_filter_beta
		self.turnrate_filter_alpha = turnrate_filter_alpha
		self.turnrate_filter_beta = turnrate_filter_beta
	...
	def parse(self,buf):
		...
		self.cf_filter_gain = struct.unpack_from('<f',buf,offset)[0]
		offset = offset + struct.calcsize('<f')

		self.pressure_drift_gain = struct.unpack_from('<f',buf,offset)[0]
		offset = offset + struct.calcsize('<f')
		...
		self.ay_filter_alpha = struct.unpack_from('<f',buf,offset)[0]
		offset = offset + struct.calcsize('<f')

		self.ay_filter_beta = struct.unpack_from('<f',buf,offset)[0]
		offset = offset + struct.calcsize('<f')

		self.turnrate_filter_alpha = struct.unpack_from('<f',buf,offset)[0]
		offset = offset + struct.calcsize('<f')

		self.turnrate_filter_beta = struct.unpack_from('<f',buf,offset)[0]
		offset = offset + struct.calcsize('<f')
	...
	def serialize(self):
		...
		buf.extend(struct.pack('<f', self.cf_filter_gain))
		buf.extend(struct.pack('<f', self.pressure_drift_gain))
		buf.extend(struct.pack('<f', self.ax_filter_alpha))
		buf.extend(struct.pack('<f', self.ax_filter_beta))
		buf.extend(struct.pack('<f', self.ay_filter_alpha))
		buf.extend(struct.pack('<f', self.ay_filter_beta))
		buf.extend(struct.pack('<f', self.turnrate_filter_alpha))
		buf.extend(struct.pack('<f', self.turnrate_filter_beta))
...
class VehicleLimits:
	...
	flightpath_angle = 0, flightpath_angle_flap = 0, unused = [None] * 10):
