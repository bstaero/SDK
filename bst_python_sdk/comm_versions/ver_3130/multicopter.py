...
class FlightControlParameters:
	...
	def __init__ (self, nav_lookahead = 0.0, wpt_capture_dist = 0.0,
	cruise_speed = 0.0, unused = [None] * 28):
...
class EstimatorParameters:
	...
	altitude_filter_Q3 = 0.0, q_pos_variance = 0.0, q_vel_variance = 0.0,
	gps_horiz_variance = 0.0, gps_vert_variance = 0.0,
	gps_velocity_variance = 0.0):
		...
		self.q_pos_variance = q_pos_variance
		self.q_vel_variance = q_vel_variance
		self.gps_horiz_variance = gps_horiz_variance
		self.gps_vert_variance = gps_vert_variance
		self.gps_velocity_variance = gps_velocity_variance
	...
	def parse(self,buf):
		...
		self.q_pos_variance = struct.unpack_from('<f',buf,offset)[0]
		offset = offset + struct.calcsize('<f')

		self.q_vel_variance = struct.unpack_from('<f',buf,offset)[0]
		offset = offset + struct.calcsize('<f')

		self.gps_horiz_variance = struct.unpack_from('<f',buf,offset)[0]
		offset = offset + struct.calcsize('<f')

		self.gps_vert_variance = struct.unpack_from('<f',buf,offset)[0]
		offset = offset + struct.calcsize('<f')

		self.gps_velocity_variance = struct.unpack_from('<f',buf,offset)[0]
		offset = offset + struct.calcsize('<f')
	...
	def serialize(self):
		...
		buf.extend(struct.pack('<f', self.q_pos_variance))
		buf.extend(struct.pack('<f', self.q_vel_variance))
		buf.extend(struct.pack('<f', self.gps_horiz_variance))
		buf.extend(struct.pack('<f', self.gps_vert_variance))
		buf.extend(struct.pack('<f', self.gps_velocity_variance))
		return bytearray(buf)
...
class VehicleParameters:
	...
	mass = 0.0, torque = 0.0, drag = 0.0, unused = [None] * 16):
		if (len(name) != 16):
