...
class CAN_GNSS_RELPOSNED:
	PACKET_TYPES = ['CAN_PKT_GNSS']
	SIZE = 57

	def __init__ (self, startByte = 0, timestamp = 0, refStationId = 0,
	iTOW = 0, relative_north = 0.0, relative_east = 0.0, relative_down = 0.0,
	relative_length = 0.0, relative_heading = 0.0, accN = 0, accE = 0, accD = 0,
	accLength = 0, accHeading = 0, flags = 0, chk = 0):
		self.startByte = startByte
		self.timestamp = timestamp
	...
	def parse(self,buf):
		if (len(buf) != self.SIZE):
			raise BufferError('INVALID PACKET SIZE [CAN_GNSS_RELPOSNED]: Expected=' + str(self.SIZE) + ' Received='+ str(len(buf)))
		...
		self.timestamp = struct.unpack_from('<I',buf,offset)[0]
		offset = offset + struct.calcsize('<I')

		self.refStationId = struct.unpack_from('<H',buf,offset)[0]
		offset = offset + struct.calcsize('<H')

		self.iTOW = struct.unpack_from('<I',buf,offset)[0]
		offset = offset + struct.calcsize('<I')

		self.relative_north = struct.unpack_from('<f',buf,offset)[0]
		offset = offset + struct.calcsize('<f')

		self.relative_east = struct.unpack_from('<f',buf,offset)[0]
		offset = offset + struct.calcsize('<f')

		self.relative_down = struct.unpack_from('<f',buf,offset)[0]
		offset = offset + struct.calcsize('<f')

		self.relative_length = struct.unpack_from('<f',buf,offset)[0]
		offset = offset + struct.calcsize('<f')

		self.relative_heading = struct.unpack_from('<f',buf,offset)[0]
		offset = offset + struct.calcsize('<f')

		self.accN = struct.unpack_from('<I',buf,offset)[0]
		offset = offset + struct.calcsize('<I')

		self.accE = struct.unpack_from('<I',buf,offset)[0]
		offset = offset + struct.calcsize('<I')

		self.accD = struct.unpack_from('<I',buf,offset)[0]
		offset = offset + struct.calcsize('<I')

		self.accLength = struct.unpack_from('<I',buf,offset)[0]
		offset = offset + struct.calcsize('<I')

		self.accHeading = struct.unpack_from('<I',buf,offset)[0]
		offset = offset + struct.calcsize('<I')

		self.flags = struct.unpack_from('<I',buf,offset)[0]
		offset = offset + struct.calcsize('<I')
	...
	def serialize(self):
		...
		buf.extend(struct.pack('<H', self.refStationId))
		buf.extend(struct.pack('<I', self.iTOW))
		buf.extend(struct.pack('<f', self.relative_north))
		buf.extend(struct.pack('<f', self.relative_east))
		buf.extend(struct.pack('<f', self.relative_down))
		buf.extend(struct.pack('<f', self.relative_length))
		buf.extend(struct.pack('<f', self.relative_heading))
		buf.extend(struct.pack('<I', self.accN))
		buf.extend(struct.pack('<I', self.accE))
		buf.extend(struct.pack('<I', self.accD))
		buf.extend(struct.pack('<I', self.accLength))
		buf.extend(struct.pack('<I', self.accHeading))
...
class CAN_ArmRemoteID:
	PACKET_TYPES = ['CAN_PKT_ARM_REMOTE_ID']
	SIZE = 4
...
class CAN_DeploymentTube:
	PACKET_TYPES = ['CAN_PKT_DEPLOYMENT_TUBE']
	SIZE = 7
...
class CAN_GCSLocation:
	PACKET_TYPES = ['CAN_PKT_GCS_LOCATION']
	SIZE = 23
...
class CAN_OperatorID:
	PACKET_TYPES = ['CAN_PKT_OPERATOR_ID']
	SIZE = 23
...
class CAN_RemoteID:
	PACKET_TYPES = ['CAN_PKT_REMOTE_ID']
	SIZE = 7
...
class CAN_RemoteIDErrorMessage:
	PACKET_TYPES = ['CAN_PKT_REMOTE_ID_ERROR_MSG']
	SIZE = 53

	def __init__ (self, startByte = 0, error_message = [None] * 50,
	chk = 0):
		self.startByte = startByte

		if (len(error_message) != 50):
			raise ValueError('array error_message expecting length '+str(50)+' got '+str(len(error_message)))
	...
	def parse(self,buf):
		if (len(buf) != self.SIZE):
			raise BufferError('INVALID PACKET SIZE [CAN_RemoteIDErrorMessage]: Expected=' + str(self.SIZE) + ' Received='+ str(len(buf)))
		...
		for i in range(0,50):
			self.error_message.append(struct.unpack_from('<B',buf,offset)[0])
			offset = offset+struct.calcsize('<B')
...
class CAN_SerialNumber:
	PACKET_TYPES = ['CAN_PKT_SERIAL_NUMBER']
	SIZE = 23
